...
function caret(key: SortKey) {
  if (sortKey.value !== key) return "ph-bold ph-arrows-down-up bom-caret-idle";
  return sortDir.value === 1 ? "ph-bold ph-caret-up" : "ph-bold ph-caret-down";
}
</script>
  ...
  <div>
    <div class="bom-toolbar">
      <div class="bom-search">
        ...
        <input v-model="query" type="text" placeholder="Filter parts..." />
      </div>
      <div class="bom-total">
          ...
          <tr v-for="(item, i) in rows" :key="item.part + i">
            ...
            <td class="bom-td-buy">
              <a
                ...
                class="bom-buy"
              >
                <i class="ph-bold ph-arrow-up-right"></i>{{ item.buyLabel || "Buy" }}
              </a>
              <span v-else class="bom-buy-none">—</span>
            </td>
          </tr>
          <tr v-if="rows.length === 0">
            <td colspan="4" class="bom-empty">No parts match "{{ query }}"</td>
          </tr>
        </tbody>
      </table>
    </div>
